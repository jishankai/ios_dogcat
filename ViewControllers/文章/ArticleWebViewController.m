...
-(void)createFakeNavigation
{
    ...
    UILabel * titleLabel = [MyControl createLabelWithFrame:CGRectMake(60, 64-20-12, 200, 20) Font:17 Text:@"我是大萌星"];
...
//    NSLog(@"萌事文章具体内容%@",self.model.url);
    
    NSString *SIDUrl = [NSString stringWithFormat:@"%@&SID=%@",self.model.url,[ControllerManager getSID]];
    [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:SIDUrl]]];
    NSLog(@"萌事文章详情具体内容SIDUrl:%@",SIDUrl);
}
...
-(void)shareClick:(UIButton *)button
{
    ...
    if(button.tag == 200){
        ...
        NSLog(@"微博");
        NSString * str = [NSString stringWithFormat:@"%@%@ #我是大萌星#", self.model.title, self.model.url];
